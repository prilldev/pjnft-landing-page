// METAMASK CONNECTION
const TIMEOUT = 1000;
const COLLECTION_NAME = 'POKERJACKS';
let editions = [];
let dots = 1;
...
function updateStatusText(isOwner, checking) {
  ...
  if(checking) {
    if(isOwner) {
      statusText.innerText = `You do own one or more ${COLLECTION_NAME}!! Let's see how many${renderDots(dots)}`;
    } else {
      statusText.innerText = `Checking to see if you own any ${COLLECTION_NAME} ${renderDots(dots)}`;
    }
  } else {
    if(isOwner) {
      statusText.innerText = `You own ${editions.length} of our ${COLLECTION_NAME}!!`;
    } else {
